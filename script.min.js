...
function _getTextShadowCSS() {
    ...
    for (var i = 0; i < l; i++) {
        ...
        if (f === "rgba") {
            console.log(f)
            colour = prop.color.toRgbString()
            console.log(colour)
        } else if (f === "hsla") {
            colour = prop.color.toHslString()
        } else if (f === "hex") {

            if (prop.color.getAlpha() !== 1) {
                colour = prop.color.toHex8String()
                const a = prop.color.toRgbString()
                console.log(tinycolor(a))
                const b = new tinycolor(a)
                console.log(b.toHex8String())
                console.log(tinycolor(prop.color.toRgbString()).toHex8String())
                console.log(colour)
            } else {
                colour = prop.color.toHexString()
            }
            console.log(prop.color.getAlpha())
        } 
        console.log(colour)
        const css = prop.offsetX + prop.offsetXUnit + " " + prop.offsetY + prop.offsetYUnit + " " + prop.blurRadius + prop.blurRadiusUnit + " " + colour
        textShadow += ", " + css 
    }
...
function createUnitType2(unit, type) {
     const elem1 = $("<div class='bg-selected'>" + unit + "</div>")
    const elem0 = $("<div style='display: inline-block'></div>")
    const values = ["px", "em", "rem", "vw", "vh"]
...
$("#font-size-unit").append(createUnitType2("px", "fontSizeUnit"))
$("#letter-spacing-unit").append(createUnitType2("px", "letterSpacingUnit"))
$("#word-spacing-unit").append(createUnitType2("px", "wordSpacingUnit"))
...
function _firstload() {
    ...
    $("#font-size").val(textshadow.fontSize)
    $("#font-size-slider").slider("value", textshadow.fontSize)
   
    const {fontFamily, letterSpacing, letterSpacingUnit, wordSpacing, wordSpacingUnit, fontWeight} = textshadow
    ...
    if (letterSpacing === "normal") {
        ...
        $("#letter-spacing-unit").text(letterSpacingUnit)
    ...
    if (wordSpacing === "normal") {
        ...
        $("#word-spacing-unit").text(wordSpacingUnit)
...
function _loadTextShadow(elementHTML, format) {
    ...
    let fontFamily, fontSizeUnit, letterSpacingUnit, wordSpacingUFamily
    fontFamily = value.g.replace(/\"|\'/g, "")

    fontSizeUnit = value.d.match(/px|em|rem|vw|vh/)
    console.log(fontSizeUnit)

    if (value.e !== "normal") {
        console.log(value.e)
        letterSpacingUnit = value.e.match(/(px|em|rem|vw|vh)/)
    ...
    if (value.f !== "normal") {
        console.log(value.f)
        wordSpacingUnit = value.f.match(/(px|em|rem|vw|vh)/)
...
$("#letter-spacing").keyup(function() {
        const value = $(this).val()
        textshadow.letterSpacing = value
        _updateTextShadowPreview()
        $("#letter-spacing-slider").slider("value", value)
})
...
$("#word-spacing").keyup(function() {
        ...
        _updateTextShadowPreview()
        $("#word-spacing-slider").slider("value", value)
})
